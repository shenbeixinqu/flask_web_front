      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <el-button
              size="mini"
              @click="previewBranch(scope.row)"
            >预览</el-button>
            <el-button
              size="mini"
              @click="delBranch(scope.row)"
            >删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
import { branchList, addBranch, deleteBranch } from '@/api/overview'
...
export default {
  ...
  methods: {
    getBranchList() {
      ...
      branchList(searchData).then(res => {
    ...
    editBranch(row) {
      ...
      this.addForm.logo = row.logo
    },
    // 提交
    addSubmit(formName) {
      this.$refs['addForm'].validate((valid) => {
        if (valid) {
          addBranch(this.addForm).then(res => {
            if (res.data.status === 200) {
    ...
    // 确认删除
    saveDeleteDialog() {
      deleteBranch(this.deleteForm).then(res => {
        if (res.data.status === 200) {
          this.$message.success(res.msg)
    ...
    handleAvatarSuccess(res, file) {
      console.log('res', res)
      this.addForm.logo = res.file_dir
    },
